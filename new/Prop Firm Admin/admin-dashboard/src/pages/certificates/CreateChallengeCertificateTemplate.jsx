...
const CreateChallengeCertificateTemplate = () => {
    ...
    const handleSubmit = (e) => {
        ...
        alert('Challenge certificate template created successfully!');
        navigate('/certificate-templates/challenge-completion');
    };
        ...
        <div className="space-y-6 animate-in fade-in zoom-in duration-300">
            ...
            <div className="glass-panel rounded-2xl border border-border overflow-hidden bg-surface">
                ...
                <div className="p-4 border-b border-border flex justify-between items-center bg-surface/50">
                    ...
                    <button
                        onClick={() => navigate('/certificate-templates/challenge-completion')}
                ...
                <form onSubmit={handleSubmit} className="p-8">
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-12">
                                ...
                                <div className="space-y-6">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div className="flex-1 flex items-center h-9">
                                            ...
                                            <input type="number" value={formData.fontSize} onChange={(e) => setFormData({ ...formData, fontSize: parseInt(e.target.value) || 0 })} className="flex-1 h-full border-t border-b border-border bg-surface text-center text-xs text-text-main focus:outline-none" />
                        ...
                        <div className="lg:col-span-5 space-y-8 pl-4">
                            <div>
                                ...
                                <div className="space-y-4">
                                    {[
                                        ...
                                        { label: 'Username Width', key: 'usernameWidth' }
                                    ].map((field) => (
                                        <div key={field.key} className="flex items-center gap-4">
                                            <label className="w-24 text-right text-[10px] font-medium text-text-muted leading-tight">
                                                <span className="text-danger mr-1">*</span>
                                                {field.label}
                                            </label>
                                            <div className="flex-1 flex items-center h-8 max-w-[240px]">
                                                <button type="button" onClick={() => handleDecrement(field.key)} className="h-full w-9 flex items-center justify-center border border-border rounded-l-lg bg-surface hover:bg-surface-lighter text-text-muted transition-colors">
                                                    <Minus className="w-3 h-3" />
                                                </button>
                                                <input
                                                    type="number"
                                                    ...
                                                    className="flex-1 h-full border-t border-b border-border bg-surface text-center text-[11px] text-text-main focus:outline-none"
                                                />
                                                <button type="button" onClick={() => handleIncrement(field.key)} className="h-full w-9 flex items-center justify-center bg-[#5d6bb3] border border-[#5d6bb3] rounded-r-lg text-white hover:bg-[#4d5999] transition-colors">
                                                    <Plus className="w-3 h-3" />
                                                </button>
                                            </div>
                                        </div>
                            ...
                            <div className="pt-4">
                                <h3 className="text-xs font-bold text-text-muted uppercase tracking-wider mb-8 text-center invisible">Plan Title Position Settings</h3>
                                <div className="space-y-4">
                                    {[
                                        { label: 'Plan Title X', key: 'planTitleX' },
                                        { label: 'Plan Title Y', key: 'planTitleY' },
                                        { label: 'Plan Title Width', key: 'planTitleWidth' }
                                    ].map((field) => (
                                        <div key={field.key} className="flex items-center gap-4">
                                            <label className="w-24 text-right text-[10px] font-medium text-text-muted leading-tight">
                                                <span className="text-danger mr-1">*</span>
                            ...
                            <div className="mt-10 flex justify-center lg:justify-start lg:pl-28">
                                <button type="button" className="flex items-center gap-2 px-6 py-2 bg-[#5d6bb3] text-white rounded text-xs font-bold hover:bg-[#4d5999] transition-all shadow-lg shadow-primary/20">
...
export default CreateChallengeCertificateTemplate;
