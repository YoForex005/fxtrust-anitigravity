...
const CreateLetKnowPayGateway = () => {
    ...
    const handleSubmit = (e) => {
        ...
        alert('LetKnowPay Gateway created successfully!');
        navigate('/payment-gateways/letknowpay');
    };
        ...
        <div className="space-y-6 animate-in fade-in zoom-in duration-300">
            ...
            <div className="glass-panel rounded-2xl border border-border overflow-hidden bg-surface">
                ...
                <div className="p-4 border-b border-border flex justify-between items-center bg-surface/50">
                    ...
                    <button
                        onClick={() => navigate('/payment-gateways/letknowpay')}
                ...
                <form onSubmit={handleSubmit} className="p-10 space-y-8 max-w-4xl mx-auto">
                    ...
                    <div className="flex items-start gap-6">
                        ...
                        <div className="flex-1">
                            <div className="w-48 h-56 bg-surface border border-border rounded-lg relative flex flex-col items-center justify-center p-4 text-center">
                                <div className="flex-1 flex items-center justify-center">
                                    <div className="w-16 h-16 bg-blue-500/10 rounded-full flex items-center justify-center">
                                        <img
                                            src="https://letknow.pay/favicon.ico"
                                            alt="letknowpay"
                                            className="w-10 h-10 opacity-60 grayscale object-contain"
                                            onError={(e) => {
                                                e.target.style.display = 'none';
                                                e.target.parentElement.innerHTML = '<div class="w-10 h-10 bg-blue-500/20 rounded flex items-center justify-center"><div class="text-[8px] font-bold text-blue-500">LKP</div></div>';
                                            }}
                                        />
                                    </div>
                                </div>
                                <div className="text-[10px] text-text-muted mb-4 font-mono">letknowpay.svg</div>
                    ...
                    <div className="flex items-center gap-6">
                        <label className="w-24 text-right text-xs font-medium text-text-muted">
                            <span className="text-danger mr-1">*</span> Shop Id
                        </label>
                        <div className="flex-1 flex items-center h-10">
                            <div className="h-full w-10 flex items-center justify-center bg-surface border border-r-0 border-border rounded-l-lg text-text-muted">
                                <Pencil className="w-3.5 h-3.5" />
                    ...
                    <div className="flex items-center gap-6">
                        <label className="w-24 text-right text-xs font-medium text-text-muted">
                            <span className="text-danger mr-1">*</span> Shop Key
                        </label>
                        <div className="flex-1 flex items-center h-10">
                            <div className="h-full w-10 flex items-center justify-center bg-surface border border-r-0 border-border rounded-l-lg text-text-muted">
                                <Pencil className="w-3.5 h-3.5" />
                            ...
                            <input
                                ...
                                onChange={(e) => setFormData({ ...formData, shopKey: e.target.value })}
                                placeholder="Input shop key"
                                className="flex-1 h-full bg-surface border border-border rounded-r-lg px-4 text-xs text-text-main focus:outline-none focus:border-primary transition-colors"
                            />
                        </div>
                    </div>
                    ...
                    <div className="flex items-center gap-6">
                        <label className="w-24 text-right text-xs font-medium text-text-muted">
                            <span className="text-danger mr-1">*</span> Asset Type
                        </label>
                        <div className="flex-1 relative">
                            <select
                                ...
                                className="w-full h-10 bg-surface border border-border rounded-lg px-4 text-xs text-text-muted focus:outline-none focus:border-primary transition-colors appearance-none"
                            >
...
export default CreateLetKnowPayGateway;
