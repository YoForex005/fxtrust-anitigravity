...
function Calendar() {
    ...
    const [selectedDays, setSelectedDays] = React.useState([new Date().toLocaleDateString('en-US', { weekday: 'short' })]);
                ...
                <table className="w-full border-collapse">
                    <thead>
                        <tr className="bg-[#f9fafb] dark:bg-[#1a1a1a]">
                            ...
                            <th className="text-right text-[10px] text-[#86868b] dark:text-[#6b7280] font-semibold px-6 py-4 border-b border-[#eee] dark:border-[#262626] uppercase tracking-wider">Actual</th>
                    ...
                    <tbody>
                        {events.filter(ev => selectedDays.includes(ev.day)).map((ev, index) => (
                            <tr key={index} className="hover:bg-[#f9fafb] dark:hover:bg-[#1a1a1a] transition-colors group">
                                ...
                                <td className="px-6 py-4 text-[13px] font-bold text-[#1d1d1f] dark:text-[#e5e7eb] border-b border-[#f5f5f5] dark:border-[#1f1f1f] align-middle text-right tabular-nums">
                                    {ev.actual ? (
