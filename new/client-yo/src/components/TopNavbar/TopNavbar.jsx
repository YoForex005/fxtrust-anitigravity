                    ...
                    {showUserDropdown && (
                        <div className="absolute top-[calc(100%+12px)] right-0 bg-white dark:bg-[#1f2937] rounded-xl shadow-[0_8px_32px_rgba(0,0,0,0.15)] min-w-[320px] z-[1000] animate-in fade-in slide-in-from-top-2 duration-200 border border-transparent dark:border-[#374151]">
                            <div className="flex gap-3 p-5 items-start">
                                <div className="w-12 h-12 bg-gradient-to-br from-blue-400 to-blue-600 rounded-full flex items-center justify-center text-white text-base font-bold shrink-0">{initials}</div>
                                <div className="flex-1">
                                    <div className="text-[15px] font-bold text-[#1a1a1a] dark:text-[#f3f4f6] mb-1">{displayName}</div>
                                    <div className="text-xs text-[#666] dark:text-[#9ca3af] mb-1">{displayEmail}</div>
                                    <div className="text-xs text-[#007bff] font-semibold">{user?.points || 0} Yo Pips Points</div>
                                </div>
                            </div>
                            ...
                            <button onClick={() => { logout(); setShowUserDropdown(false); }} className="flex items-center justify-center gap-2 p-3.5 m-3 mt-2 bg-gradient-to-br from-[#007bff] to-[#0056b3] text-white rounded-lg text-sm font-semibold cursor-pointer transition-all hover:-translate-y-0.5 hover:shadow-[0_4px_12px_rgba(0,123,255,0.3)] no-underline w-full border-none">
                                <LogOut size={20} />
                                <span>Log Out</span>
                            </button>
                        </div>
