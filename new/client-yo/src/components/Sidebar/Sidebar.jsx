...
function Sidebar({ darkMode, toggleTheme, openChat, closeSidebar }) {
    ...
    return (
        <div className="w-[260px] bg-white dark:bg-[#0d0d0d] flex flex-col border-r border-[#f0f0f0] dark:border-[#1a1a1a] z-10 h-full overflow-hidden">
            {/* Logo Section */}
            {/* Logo Section */}
            <div className="px-6 py-6 border-b border-gray-100 dark:border-[#1a1a1a]">
                <Link to="/" className="flex items-center gap-3 no-underline group" onClick={closeSidebar}>
                    <div className="relative">
                        ...
                        <img
                            src="/logo.png"
                            alt="Yo Pips Logo"
                            className="w-10 h-10 rounded-xl object-contain relative z-10 group-hover:scale-105 transition-transform"
                        />
                    </div>
                    <span className="text-xl font-bold text-[#1d1d1f] dark:text-white tracking-tight group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors">Yo Pips</span>
                </Link>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-3 py-4">
                ...
                {/* CTA Button */}
                <button
                    className="w-full bg-gradient-to-r from-blue-600 via-blue-500 to-indigo-600 text-white border-none rounded-2xl p-4 font-bold text-sm cursor-pointer mb-8 transition-all hover:shadow-xl hover:shadow-blue-500/30 hover:-translate-y-0.5 flex items-center justify-center gap-2 group relative overflow-hidden"
                ...
                <div className="mb-5">
                    ...
                    <nav className="space-y-0.5">
                        {[
                            ...
                            { path: '/journal', icon: BookOpen, label: 'Trading Journal' },
                            ...
                            <button
                                key={path}
                                className={`w-full flex items-center gap-3.5 px-4 py-3 border-none text-[13px] font-bold cursor-pointer rounded-2xl transition-all duration-200 text-left ${isActive(path)
                                    ? 'bg-[#1d1d1f] dark:bg-white text-white dark:text-[#1d1d1f] shadow-lg shadow-black/5 dark:shadow-white/10'
                                    : 'bg-transparent text-[#6b7280] dark:text-[#9ca3af] hover:bg-gray-50 dark:hover:bg-[#1a1a1a] hover:text-[#1d1d1f] dark:hover:text-white'
                                    }`}
                                onClick={() => handleNav(path)}
                            >
                                <Icon size={18} className={`shrink-0 transition-colors ${isActive(path) ? 'text-white dark:text-[#1d1d1f]' : 'text-[#86868b] dark:text-[#6b7280] group-hover:text-[#1d1d1f] dark:group-hover:text-white'}`} />
                                <span className="tracking-wide">{label}</span>
                                {isActive(path) && <ChevronRight size={14} className="ml-auto opacity-80" />}
                            </button>
                        ))}
                    </nav>
                </div>
                    ...
                    <div className={`overflow-hidden transition-all duration-300 ${toolsOpen ? 'max-h-[500px] opacity-100' : 'max-h-0 opacity-0'}`}>
                        <div className="pl-2 pt-1 space-y-0.5">
                            ...
                            {[
                                ...
                                <button
                                    key={path}
                                    className={`w-full flex items-center gap-3.5 px-4 py-2.5 border-none text-[13px] font-bold cursor-pointer rounded-2xl transition-all duration-200 text-left ${isActive(path)
                                        ? 'bg-[#1d1d1f] dark:bg-white text-white dark:text-[#1d1d1f] shadow-md'
                                        : 'bg-transparent text-[#6b7280] dark:text-[#9ca3af] hover:bg-gray-50 dark:hover:bg-[#1a1a1a] hover:text-[#1d1d1f] dark:hover:text-white'
                                        }`}
                                    onClick={() => handleNav(path)}
                                >
                                    <Icon size={16} className={`shrink-0 transition-colors ${isActive(path) ? 'text-white dark:text-[#1d1d1f]' : 'text-[#86868b] dark:text-[#6b7280] group-hover:text-[#1d1d1f] dark:group-hover:text-white'}`} />
                                    <span className="tracking-wide">{label}</span>
                                </button>
                            ))}
                ...
                <div className="mb-5">
                    ...
                    <div className={`overflow-hidden transition-all duration-300 ${supportOpen ? 'max-h-[200px] opacity-100' : 'max-h-0 opacity-0'}`}>
                        <div className="pl-2 pt-1 space-y-0.5">
                            <button
                                className={`w-full flex items-center gap-3.5 px-4 py-2.5 border-none text-[13px] font-bold cursor-pointer rounded-2xl transition-all duration-200 text-left ${isActive('/support/send-message')
                                    ? 'bg-[#1d1d1f] dark:bg-white text-white dark:text-[#1d1d1f] shadow-md'
                                    : 'bg-transparent text-[#6b7280] dark:text-[#9ca3af] hover:bg-gray-50 dark:hover:bg-[#1a1a1a] hover:text-[#1d1d1f] dark:hover:text-white'
                                    }`}
                                onClick={() => handleNav('/support/send-message')}
                            >
                                <MessageSquare size={16} className={`shrink-0 transition-colors ${isActive('/support/send-message') ? 'text-white dark:text-[#1d1d1f]' : 'text-[#86868b] dark:text-[#6b7280]'}`} />
                            ...
                            <button
                                className="w-full flex items-center gap-3 px-3 py-2 border-none text-[13px] font-medium cursor-pointer rounded-lg transition-all duration-200 text-left bg-transparent text-[#6b7280] dark:text-[#9ca3af] hover:bg-[#5865F2]/10 hover:text-[#5865F2]"
                                onClick={() => window.open('https://discord.gg/yopips', '_blank')}
                            >
                                <MessageSquare size={16} className="shrink-0" />
                                <span>Discord</span>
                                <span className="ml-auto text-[10px] px-1.5 py-0.5 bg-[#5865F2]/10 text-[#5865F2] rounded font-bold">JOIN</span>
                            </button>
                        </div>
                    </div>
                </div>
