        ...
        <div className="flex items-center justify-between mb-6 h-auto p-0 bg-transparent mt-4 w-full">
            ...
            <div className="flex items-center gap-8">
                <div className="relative" ref={dropdownRef}>
                    ...
                    >
                        <span className="text-sm font-extrabold text-[#1e293b] dark:text-[#f3f4f6]">{displayName}</span>
                        <div className="w-8 h-8 bg-[#1e293b] text-white rounded-full flex items-center justify-center text-[11px] font-extrabold shadow-sm">{initials}</div>
                    </div>
                    ...
                    {showUserDropdown && (
                        <div className="absolute top-[calc(100%+12px)] right-0 bg-white dark:bg-[#1f1f1f] rounded-xl shadow-[0_8px_32px_rgba(0,0,0,0.15)] dark:shadow-[0_8px_32px_rgba(0,0,0,0.4)] min-w-[320px] z-[1000] animate-[dropdownSlide_0.2s_ease-out] text-left">
                            <div className="flex gap-3 p-5 items-start">
                                <div className="w-8 h-8 bg-[#1e293b] text-white rounded-full flex items-center justify-center text-[11px] font-extrabold shadow-sm shrink-0">{initials}</div>
                                <div className="flex-1">
                                    <div className="text-[15px] font-bold text-[#1a1a1a] dark:text-white mb-1">{displayName}</div>
                                    <div className="text-xs text-[#666] dark:text-[#9ca3af] mb-1">{displayEmail}</div>
                                    <div className="text-xs text-[#000] dark:text-white font-semibold">{user?.points || 0} Yo Pips Points</div>
                                </div>
                            </div>
                            ...
                            <Link to="/profile" className={dropdownItemClass} onClick={() => setShowUserDropdown(false)}>
                                <User size={20} className="text-[#1d1d1f] dark:text-white shrink-0 mt-0.5" />
                                <div>
                            ...
                            <Link to="/billing" className={dropdownItemClass} onClick={() => setShowUserDropdown(false)}>
                                <CreditCard size={20} className="text-[#1d1d1f] dark:text-white shrink-0 mt-0.5" />
                                <div>
                            ...
                            <Link to="/personalisation" className={dropdownItemClass} onClick={() => setShowUserDropdown(false)}>
                                <Settings size={20} className="text-[#1d1d1f] dark:text-white shrink-0 mt-0.5" />
                                <div>
                            ...
                            <button onClick={() => { logout(); setShowUserDropdown(false); }} className="flex items-center justify-center gap-2 p-3 mx-3 mb-3 mt-2 bg-[#1d1d1f] dark:bg-white text-white dark:text-[#1d1d1f] rounded-lg no-underline text-sm font-semibold cursor-pointer transition-all hover:opacity-90 hover:shadow-md border-none w-auto">
                                <LogOut size={20} />
                                <span>Log Out</span>
                            </button>
                        </div>
